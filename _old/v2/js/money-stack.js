(function($, undefined) {
    ...
    MoneyStack.prototype = {
        ...
        buildStackItem: function(src) {
            var item = this.ITEM__TPL.clone().attr('src', src).appendTo(this.substrate);
            return item.height((item.height() * this.scaleRatio) + 'px');
        },

        addBuilderToRenderQueue: function() {
        ...
        render: function() {
            ...
            (function() {
                ...
                if (stackItemData.count--) {
                    // This iteration render stack item of current stack type
    ...
    $.fn.MoneyStack = function(arg) {
        if (!this.data('moneyStackInstance')) {
            MoneyStack.prototype.ITEM__TPL = $('<img class="stack-item"/>');
